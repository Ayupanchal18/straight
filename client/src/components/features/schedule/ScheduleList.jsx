...
const parseMatchMeta = (matchStr = '') => {
  ...
  // Format badge colors
  const formatColors = {
    T20: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30',
    ODI: 'bg-blue-500/10 text-blue-400 border-blue-500/30',
    TEST: 'bg-amber-500/10 text-amber-400 border-amber-500/30',
    MATCH: 'bg-slate-700/30 text-slate-300 border-slate-600/30',
  };
...
export const ScheduleList = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto px-2 sm:px-4">
      {/* ── Top Tabs & Header ── */}
      <div className="bg-[#0b1120] border border-white/5 rounded-xl p-4 sm:p-6 shadow-xl backdrop-blur-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            {/* Format Filter Pills */}
            <div className="flex items-center gap-1 bg-navy-950/60 p-1 rounded-lg border border-white/5">
            ...
            <div className="relative w-full sm:w-60">
              ...
              <input
                ...
                className={`w-full pl-8 ${searchQuery ? 'pr-7' : 'pr-3'} py-1.5 bg-[#070b14] border border-white/10 rounded-lg text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors`}
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-white/10 text-slate-400"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
            ...
            <Button
              ...
              className="bg-navy-900 border-white/10 hover:border-white/20 text-white text-xs"
            >
      ...
      <div className="flex items-center justify-between text-xs text-slate-400 px-1">
        <span className="font-medium">
          Showing <span className="text-white font-bold">{filtered.length}</span> scheduled match{filtered.length === 1 ? '' : 'es'}
        </span>
        {selectedFormat !== 'ALL' && (
          <span className="text-blue-400 font-semibold">Filtered by {selectedFormat} format</span>
        )}
      </div>

      {/* Error state */}
      {error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-red-300 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
      ...
      {loading && schedule.length === 0 && (
        <div className="space-y-3">
          <Skeleton count={5} className="h-20 rounded-xl bg-navy-900/60" />
        </div>
      )}
      ...
      {!loading && Object.keys(groupedSchedule).length > 0 ? (
          ...
          {Object.entries(groupedSchedule).map(([dateLabel, items]) => (
            <div key={dateLabel} className="space-y-2.5">
              ...
              <div className="flex items-center gap-2.5 px-2">
                ...
                <h2 className="text-xs sm:text-sm font-black text-slate-300 uppercase tracking-wider font-display">
                  {dateLabel}
                </h2>
              ...
              {/* Match Rows */}
              <div className="bg-[#0b1120] border border-white/5 rounded-xl divide-y divide-white/5 overflow-hidden shadow-lg">
                {items.map((item) => {
                  ...
                  return (
                    <div
                      key={item._uniqueId}
                      className="p-3.5 sm:p-4 hover:bg-white/[0.02] transition-colors flex flex-col md:flex-row md:items-center justify-between gap-3.5"
                    >
                      {/* Left: Format + Timing */}
                      <div className="flex items-center gap-3 min-w-[140px]">
                        ...
                        {item.time && item.time !== 'TBA' ? (
                          ...
                          <span className="text-[11px] font-mono text-slate-500">TBA</span>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-sm sm:text-base font-bold text-white hover:text-blue-400 transition-colors cursor-pointer truncate">
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-3 pt-2 md:pt-0 border-t border-white/5 md:border-t-0">
                        {item.venue && (
                          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 max-w-[200px] truncate" title={item.venue}>
        ...
        <div className="bg-[#0b1120] border border-white/5 rounded-xl p-12 text-center shadow-lg">
